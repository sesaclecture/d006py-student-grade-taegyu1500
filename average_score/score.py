...
def subject_average(student_scores: dict, subjects: list):
    ...
    """
    return dict(zip(subjects, [sum(map(int, subject_scores)) / len(subject_scores) for subject_scores in zip(*student_scores.values())]))
...
def student_average(student_scores: dict):
    ...
    """
    return sorted(((name, sum(map(int, scores)) / len(scores)) for name, scores in student_scores.items()), key=lambda x: x[1], reverse=True)
